...
from weather_cmd.api.noaa import (
    fetch_alerts,
    fetch_county,
    fetch_detailed_forecast,
    fetch_forecast_fallback,
    fetch_text_forecast,
)
...
class WeatherApp(App):
            ...
            async with httpx.AsyncClient() as client:
                ...
                try:
                    ...
                    if loc.country == "United States":
                        current, hourly, daily = await fetch_forecast_fallback(
                            loc.latitude, loc.longitude, client, self._units
                        )
                    else:
                        raise
                ...
                detailed_forecast = await fetch_detailed_forecast(loc.latitude, loc.longitude, client)
    ...
    def _update_widgets(self) -> None:
        ...
        self.query_one(DailyView).update_data(data, self._units)
        self.query_one(RadarView).update_data(data.radar_image, data.location)
